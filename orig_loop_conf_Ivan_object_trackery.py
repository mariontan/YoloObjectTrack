...
config_path = r'.\config/yolov3_orig.cfg' #img_size = 416

weights_path=r'./config/yolov3_orig.weights'

class_path =r'.\config/coco_orig.names'
...
for conf_thres in confArr:
    ...
    for vid in videoArr:
        ...
        outvideo = cv2.VideoWriter(videopath.replace(".mp4", "-detOrigWeight"+"_"+str(conf_thres)+"nms"+str(nms_thres)+".mp4"),fourcc,20.0,(vw,vh))

        frames = 0
        starttime = time.time()
        car_count = 0

        file1 = open(r"D:\Ivan\origWeightCarsCount.txt","a")
        ...
        while(True):
            ...
            if detections is not None:
                ...
                for x1, y1, x2, y2, obj_id, cls_pred in tracked_objects:
                    ...
                    if((cls_pred==carClass or cls_pred==truckClass) and (obj_id in carObjId)==False and lineY <= y1+box_h ):
